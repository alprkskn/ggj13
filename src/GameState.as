	...
	public class GameState extends FlxB2State 
	{
		...
		override public function update():void 
		{
			...
			for (var i:int = 0; i < 6; i++)
			{
				fovRayObject = null;
				const mr:Number = 12;
				var fov:Number = (i - 2.5) / (5 + Math.abs(Math.sin(player.runCounter*0.2)));
			...
			fovShape.graphics.drawCircle(player.getMidpoint().x, player.getMidpoint().y, 10);
		...
		override public function draw():void 
		{
			...
			foreground.buffer.draw(fovShape);
